...
object FirJsExportedActualMatchExpectChecker : FirBasicDeclarationChecker(MppCheckerKind.Platform) {
    ...
    override fun check(declaration: FirDeclaration) {
        ...
        if (!correspondingExpectDeclaration.isDeclarationExportedToJs()) return
    ...
    private fun FirBasedSymbol<*>.isReachableOutsideOfKotlin(): Boolean =
        when (this) {
            is FirClassLikeSymbol -> isDeclarationExportedToJs() || isEffectivelyExternal()
            else -> isDeclarationExportedToJs()
        }
}
