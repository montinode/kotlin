    ...
    override fun doTargetSpecificProcessing() {
        ...
        project.launchInStage(AfterEvaluateBuildscript) {
            ...
            javaSourceSet.await()?.let { java ->
                ...
                project.tasks.withType<GroovyCompile>().matching { it.name == java.getCompileTaskName("groovy") }
                    .configureEach { groovyCompile ->
                        groovyCompile.dependsOn(kotlinTask)
                        groovyCompile.classpath += project.files(kotlinTask.flatMap { it.destinationDirectory })
                    }
