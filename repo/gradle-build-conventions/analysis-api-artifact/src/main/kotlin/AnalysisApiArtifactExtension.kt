    ...
    fun content(block: ArtifactContentBuilder.() -> Unit): Unit = with(project) {
        ...
        val builder = object : ArtifactContentBuilder {
            override fun project(path: String, isTransitive: Boolean) {
                includedProjects.compute(path) { _, oldValue -> (oldValue ?: false) || isTransitive }
            }
        }

        block(builder)

        val artifactContent = configurations.create("artifactContent")
        configurations.named("embedded").configure { extendsFrom(artifactContent) }
        ...
        sourcesJar {
            for (projectPath in includedProjects.keys) {
                project(projectPath).tasks.named("classes").configure classes@{
                    // Ensure that all code generation tasks are triggered
                    this@sourcesJar.dependsOn(this@classes)
                }
            }

            addEmbeddedSources(artifactContent.name)
            addEmbeddedLibrarySources(artifactContent)
        }
